(ns repairshop.operation
  "OperationActor -- one electronics-repair operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (RepairOps-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Repair Shop Governor (:govern) and the
  ...
  One graph run = one electronics-repair operation (intake -> advise ->
  ...
  decision to a human operator (the repair technician/shop owner). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:repair/complete`/`:device/return` ALWAYS reach this
  node when the governor is clean -- see `repairshop.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [repairshop.repairopsllm :as repairopsllm]
...
(defn build
  ...
  `repairshop.store/Store`).
  opts:
    :advisor      -- a `repairshop.repairopsllm/Advisor` (default: mock-advisor)
